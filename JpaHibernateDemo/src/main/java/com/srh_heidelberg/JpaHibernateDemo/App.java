...
public class App {
	public static void main(String[] args) {
		...
		int CodeRunValue = 1; // 1 -> for select 2 -> for Insert 3 -> for Update 4 -> for Delete 
		...
		if (CodeRunValue == 1) {
			...
			student stuUpdate = em.find(student.class, 1);
			...
			student stuDelete = em.find(student.class, 2);

			em.getTransaction().begin(); 		// need to begin the transaction before persist
			em.persist(stuDelete); 				// persist will save value in app memory
			em.getTransaction().commit(); 		// to save and reflect value into database.
			System.out.println(stuDelete);
